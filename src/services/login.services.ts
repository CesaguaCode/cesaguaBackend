...
export default class LoginService extends BaseService {
  ...
  public async getExistEmail(email: string) {
    const result: any = await this.db.executeProcedure(PROCEDURES.EXIST_EMAIL, [email]);
    return result;
  }
   ...
   */
    public async getPassword(id: number) {
        const result: any = await this.db.executeProcedure(PROCEDURES.OBTAIN_PASS, [id]);
        return result;
    }
   ...
   */
    public async getAuthData(id: number) {
        const result: any = await this.db.executeProcedure(PROCEDURES.OBTAIN_AUTH, [id]);
        return result;
    }
